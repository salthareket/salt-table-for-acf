(function($) {
	...
	function tableCellPreview(cellData) {
	    ...
	    if ((type == "boolean" || type == "text_boolean")) {
	        htmlContent += '<div class="acf-table-boolean-preview">' + (cellData.boolean?"✔":"❌") + '</div>';
	    }
	...
	function initModalMCE(modal, content) {
	    ...
	    // TinyMCE hazır olunca content ata
	...
	// Modal açıldığında veya dropdown değiştiğinde çağrılacak
	function updateCellTypeFields(modal) {
	    ...
	    // Sadece seçilen tipe ait alanları göster
	    switch (type) {
	        case 'text':
	...
	function SaltACFTableField() {
		...
		t.init = function() {
			...
			// 1. Modal HTML içine cell_type dropdown ekle
				        ...
				        <div id="acf-table-content-modal" class="acf-table-modal" style="display:none;">
				            ...
				            <div class="acf-table-modal-inner">
				                <div class="acf-table-modal-content acf-fields-wrapper">
				                    <div class="acf-field acf-field-cell-type">
				                        ...
				                        <select class="acf-table-cell-type">
				                            ...
				                            <option value="text_boolean">Text & Boolean</option>
				                        </select>
				                    </div>
				                    ...
				                    <div class="acf-table-field acf-field acf-table-image-field">
				                        ...
				                        <button type="button" class="acf-table-select-image button">Select Image</button>
				                        <div class="acf-table-image-preview"></div>
				                        <button type="button" class="acf-table-remove-image button" style="display:none;">Remove</button>
				                    </div>
				                    <div class="acf-table-field acf-field acf-table-file-field">
				                        ...
				                        <button type="button" class="acf-table-select-file button">Select File</button>
				                        <div class="acf-table-file-preview"></div>
				                        <button type="button" class="acf-table-remove-file button" style="display:none;">Remove</button>
				                    </div>
				                ...
				                <div class="acf-table-modal-footer">
				                    <button type="button" class="acf-table-modal-save button button-primary">Kaydet</button>
				                    <button type="button" class="acf-table-modal-close button">Kapat</button>
				                </div>
				            </div>
				        </div>
				    `);
			...
			// }
				...
				// Delegated event binding ile açma
				t.obj.body.on('click', '.acf-table-body-cell, .acf-table-header-cell', function(e){
				    ...
				    // Hücre verilerini modal alanlarına yükle
		...
		t.cell_editor_modal = function(){
		    t.obj.body.on('click', '.acf-table-body-cell, .acf-table-header-cell', function(e){
		       ...
		       // t.cell_editor_save && t.cell_editor_save();
		        ...
		        // Hücre datasını al
		        console.log(that.attr('data-json'))
		...
		t.cell_editor_modal_events = function(){
		    var modal = $('#acf-table-content-modal');
		    console.log("cell_editor_modal_events");
		    console.log(modal)
			...
			modal.on('click', '.acf-table-modal-save', function() {
			   ...
			   // t.state.current_cell_obj.attr('data-json', JSON.stringify(cellData));
			    ...
			    // JSON ile tabloyu güncelle ve DB'ye kaydet
			    var root = t.state.current_cell_obj.closest('.acf-table-root');
			    var p = {};
			    p.obj_root = root;
			    p.obj_table = root.find('.acf-table-table');

			    // İşte eski çalışan fonksiyon: tüm table JSON'unu oluşturuyor ve input'a yazıyor
		...
		t.table_build_json_from_data = function(root){
		    ...
		    // root’a attribute olarak kaydet (veya ajax ile backend’e)
		    console.log(root)
		...
		t.table_render = function(p) {
		    ...
		    // BODY ROWS CELLS (DATA-JSON EKLİ)
			    ...
			    body_rows.each(function() {
			        ...
			        for (i in p.data.b[row_i]) {
			            ...
			            if(cellCont.length) {
			                ...
			                // Eğer yoksa (yeni eklenen hücre) yarat
			                console.log(t.param.htmltable.body_cell)
		...
		t.table_build_json = function(p) {
		    ...
		    p.obj_table.find('.acf-table-header-cell').each(function(i) {
		        ...
		        let json = getCellData(cell);
		           console.log("katdeeet hedır", json);
		            if (json) {
		                try {
		                    cellData = json;
		                } catch(e) {
		    ...
		    // eksik header hücrelerini doldur
		...
		/*t.update_table_data_field = function(p) {
		    ...
		    if (typeof t.field_changed === 'function') {
		        ...
		        if (isPostEditor) {
		            ...
		            console.warn('ACF field_changed atlanıyor: postType config yok');
		        }
		    }
		};*/
